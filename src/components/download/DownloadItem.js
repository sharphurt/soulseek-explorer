...
import {formatTransferSpeed, getBaseFilename} from '../../utils/formatters.js';
import {getProgress} from "../../utils/getProgress.js";
import {getStatusMeta} from "../../utils/getStatusMeta.js";
import ProgressBar from "./ProgressBar.js";

const DownloadItem = ({download}) => {
    const filename = download.filename || download.fileName || download.path || '';
    const displayName = getBaseFilename(filename);
        ...
        <article className="rounded-lg border border-white/10 bg-slate-950/45 p-3 shadow-lg shadow-black/10">
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-slate-100" title={filename}>
                        {displayName}
                    </p>
